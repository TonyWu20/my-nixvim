...
local opts = {
	...
	sources = {
		default = { "lazydev", "lsp", "snippets", "path", "buffer", "ripgrep", "latex_symbols" },
		providers = {
			lsp = { max_items = 350 },
			lazydev = {
			...
			latex_symbols = {
				name = "LaTeX",
				-- Native blink source (no nvim-cmp needed). Serves the
				-- vendored LaTeX symbol data; see completion/latex_symbols_data.lua.
...
return function()
	require("utils").load_plugin("blink.cmp", opts)
end
